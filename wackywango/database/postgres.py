...
class PostgresDriver:
    ...
    def __init__(self, url):
        ...
        self.snapshots_data = db.Table('snapshots_data', metadata,
                        db.Column('id', db.Integer, primary_key=True),
    ...
    def get_snapshot_data(self,user_id,snapshot_id,result_name):
        # Get the snapshot - For "security" reasons it might be worth adding a validation here that the snapshot id matches the user_id
        # query = db.select([self.snapshots.c.id, self.snapshots.c.snapshot_timestamp]).where(and_(self.snapshots.columns.id == snapshot_id,self.snapshots.columns.user_id == user_id))
        # ResultProxy = self.connection.execute(query)
        # snapshot_ResultSet = ResultProxy.fetchall()

        # Get the snapshot data
        query = db.select([self.snapshots_data.c.data]).where(and_(self.snapshots_data.columns.snapshot_id == snapshot_id,self.snapshots_data.columns.parser_type == result_name))
        ResultProxy = self.connection.execute(query)
        ResultSet = ResultProxy.fetchall()
    ...
    def add_snapshot(self,user_id,snapshot_timestamp,parser_type,data):
        ...
        query = db.select([self.snapshots]).where(and_(self.snapshots.columns.user_id == user_id,self.snapshots.columns.snapshot_timestamp == str(snapshot_timestamp)))
        ResultProxy = self.connection.execute(query)
        ResultSet = ResultProxy.fetchall()
        snapshot_id = 0
        if not ResultSet:
