    ...
    def parse(self, parser_type, data):
        ...
        for parser in self.parsers:
            if parser.parser_type == parser_type:
                ...
                collect_results.append(context.prepare_result(result,upload_snapshot.snapshot.datetime))
...
class Context:
    ...
    def prepare_result(self,data,snapshot_timestamp):
        if not self.result:
            ...
            return {"snapshot_timestamp":snapshot_timestamp,"parser_type":self.parser_type,"snapshot_data":data,"user_data":user_data}
        else:
            return self.result
...
def parse(parser_type,data):
    return parser.parse(parser_type,data)
...
def run_parser(parser_type,url):
    def callback(channel, method, properties, body):
        ...
        for result in results:
            queue.publish('exchange', 'parsed.'+queue_data['parser_type'], result)
